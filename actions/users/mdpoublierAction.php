    ...
    if (!empty($_POST['Email'])){
        ...
        if (filter_var($user_email,FILTER_VALIDATE_EMAIL)){
            ...
            if ($checkIfEmailExiste->rowCount() > 0){
                ...
                $optenircode =$bdd->prepare('SELECT code FROM recuperation WHERE email=?');
                $optenircode->execute(array($user_email));
                $code_optenu = $optenircode->fetch();
                $_SESSION['recup_code']= $code_optenu['code'];
...
if (isset($_POST['verif-cade'],$_POST['verif-submit'])){
    if (!empty($_POST['verif-cade'])){
        ...
        $verif_req->execute(array($_SESSION ['recup_mail'],$verif_cade));// $_SESSION ['recup_code']
